import { ORIGAMI_MODELS } from './data/origamiModels.js';
...
let currentModel = ORIGAMI_MODELS[0];
...
modelSelect.addEventListener('change', (e) => {
  const selected = ORIGAMI_MODELS.find(m => m.id === e.target.value);
  if (selected) {
    loadModel(selected);
  }
});
